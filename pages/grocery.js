...
const Grocerry = ( { products } ) => {
  return (
    <div>
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys( products ).length == 0 && <p>Sorry! grocerry section Of our stock is empty.New stock coming soon, Stay Tuned..</p>}
...
export async function getServerSideProps( context ) {
  ...
  let products = await Product.find( { catagory: 'grocerry' } )
  let grocerry = {}
  for ( let item of products ) {
    if ( item.title in grocerry ) {
      if ( !grocerry[item.title].color.includes( item.color ) && item.availableQty > 0 ) {
        grocerry[item.title].color.push( item.color )
      }
      if ( !grocerry[item.title].size.includes( item.size ) && item.availableQty > 0 ) {
        grocerry[item.title].size.push( item.size )
      }
    }
    else {
      grocerry[item.title] = JSON.parse( JSON.stringify( item ) )
      if ( item.availableQty > 0 ) {
        grocerry[item.title].color = [item.color]
        grocerry[item.title].size = [item.size]
      }
    }
  }
  return {
    props: { products: JSON.parse( JSON.stringify( grocerry ) ) }
  }
}
