      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys( products ).length == 0 && <p>Sorry! Fashion section Of our stock is empty.New stock coming soon, Stay Tuned..</p>}
...
export async function getServerSideProps( context ) {
  ...
  let products = await Product.find( { catagory: 'fashion' } )
  let fashion = {}
  for ( let item of products ) {
    if ( item.title in fashion ) {
      if ( !fashion[item.title].color.includes( item.color ) && item.availableQty > 0 ) {
        fashion[item.title].color.push( item.color )
      }
      if ( !fashion[item.title].size.includes( item.size ) && item.availableQty > 0 ) {
